            ...
            <form action="{{ route('bantuanmodal.monitoring.update', $monitoring->id) }}" method="post"
                        ...
                        <div class="col-sm-8">
                            <select name="tahun_monitoring" id="" class="form-select">
                                @foreach (range(date('Y'),2022) as $year)
                                    <option value="{{$year}}" @selected($monitoring->tahun_monitoring==$year)>{{$year}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="row app-card shadow-sm bg-white p-3 mt-4">
                    <div class="app-card-body">
                            ...
                            <center>
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="status_penggunaan_bantuan"
                                        id="RadioOptions1" value="1" onchange="showStatus(this)" @checked($monitoring->getRawOriginal('status_penggunaan_bantuan')==1)>
                                    <label class="form-check-label" for="RadioOptions1">Sudah Berjalan</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="status_penggunaan_bantuan"
                                        id="RadioOptions2" value="2" onchange="showStatus(this)" @checked($monitoring->getRawOriginal('status_penggunaan_bantuan')==2)>
                                    <label class="form-check-label" for="RadioOptions2">Belum Digunakan</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="status_penggunaan_bantuan"
                                        id="RadioOptions3" value="3" onchange="showStatus(this)" @checked($monitoring->getRawOriginal('status_penggunaan_bantuan')==3)>
                        ...
                        <div id="form-belum-digunakan" class="{{$monitoring->getRawOriginal('status_penggunaan_bantuan') == 1 ? 'd-none' : ''}}">
                            @include('monitoringBantuanModal.transaksi.form_edit.form_belum_digunakan')
                        </div>

                        {{-- Pengelolaan Modal Usaha --}}
                        <div id="form-modal-usaha" class="{{$monitoring->getRawOriginal('status_penggunaan_bantuan') > 1 ? 'd-none' : ''}}">
